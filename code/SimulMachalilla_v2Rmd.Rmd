...
```{r graph2, echo=TRUE, cache=TRUE, fig.cap="Figura 2. Dos formas de mostrar la relación entre la probabilidad de ocurrencia de los venados y las co-variables.",fig.scap="fig2", warning=FALSE}
...
```{r gpagh3,cache=TRUE, fig.cap="este se supone es el caption", warning=FALSE}
...
```{r graph5,cache=TRUE, warning=FALSE}
...
```{r graph6,cache=TRUE, warning=FALSE}
par(mfrow = c(2, 2), mar = c(5,4,2,2), cex.main = 1)
...
Figura 6. Relación entre la (jittered) ocupación observada de venados (y) y las tres co variables estandarizadas. Altitud (A). Cobertura de bosque (B). Temperatura (C) y la frecuencia de distribución de la ocurrencia observada (y) en un set de datos de 60 sitios con 30 muestreos cada uno (D).
...
```{r function,cache=TRUE, warning=FALSE}
...
```{r funcall2,cache=TRUE, warning=FALSE}
simrep <- 10000
...
Esta vez recurriremos a un diseño tipo TEAM (http://www.teamnetwork.org) con 60 sitios de muestreo y treinta días de muestreos repetidos. De nuevo nuestra especie es el venado de cola blanca. Para este ejemplo asumiremos que la detección es 0.5, la ocupacion 0.6 y las interacciones son mucho mas sencillas con la altitud como la unica covariable que explica la ocupación y la detección, pero en diferentes direcciones. Observe el signo positivo y negativo en los coeficientes.

```{r unmarked_data,cache=TRUE}
# data generation
# generemos un modelo sencillo donde la ocupacion esta explicada por la elevacion
datos2<-data.fn(M = 60, J = 30, beta1 = -2, beta2 = 0, beta3 = 0, 
                mean.occupancy = 0.6, mean.detection = 0.5, 
                    alpha1 = 2.5, alpha2 = -0.5, alpha3 = 0, show.plot = FALSE)
...
parboot(fm5) # goodness of fit
plot(parboot(fm5)) # goodness of fit

```


Ahora vamos a construir mapas para cada una de nuestras covariables. Los mapas surgen de un patron aleatoreo de puntos con distribucion Poisson. Luego estos puntos los convertimos en una superficie interpolada.

```{r spatial1,cache=TRUE, warning=FALSE}
...
elev <- density(rpoispp(lambda=0.01, win=window)) # 
forest <- density(rpoispp(lambda=0.02, win=window)) # 
temp <- density(rpoispp(lambda=0.5, win=window)) # 

# Put the covs in the same file and convert to raster
...
Una vez tenemos nuestros mapas de covariables, los usamos para predecir con el mejor modelo y asi tener un mapa (predicho) de la ocupacion y la probabilidad de deteccion.

```{r spatial2,cache=TRUE, warning=FALSE}
##### make the predictions
predictions_psi <- predict(fm5, type="state", newdata=mapdata.m)
predictions_p <- predict(fm5, type="det", newdata=mapdata.m)
## put in the same stack
predmaps<-stack(predictions_psi$Predicted,predictions_p$Predicted) 
names(predmaps)<-c("psi predicted", "p predicted") # put names
plot(predmaps)
...
```{r bayesian, eval=FALSE, cache=TRUE, warning=FALSE}
...
for(k in 1:2){              # 2 detection covariates
...
for(k in 1:1){             # 1 occupancy covariate terms in design mat
...
for (i in 1:M) {
  ...
  logit(psi[i]) <- beta0 +                 # occupancy (psi) intercept
    ...
    #beta[3] * temp[i] +                    # temp
    #beta[4] * elev[i] * forest[i] +        # elev:forest
    #beta[5] * elev[i] * temp[i] +          # elev:temp
...
#### Compare estimates from ML method (first two columns) and posterior sampling (column 4-5).
tmp <- summary(fm5)
modestimates <- cbind(rbind(tmp$state[1:2], tmp$det[1:2]), 
                Post.mean = out22$summary[c(3, 7, 4:6), 1],
                Post.sd = out22$summary[c(3, 7, 4:6), 2] )
real<- rbind(datos2$mean.occupancy, datos2$beta1, datos2$mean.detection,
             datos2$alpha1, datos2$alpha2)
### see if the values are close to real values
# cbind(real, modestimates) ### see if the values are close to real values

```

## Comparando los valores reales y los estimados de ML y Bayes

Veamos que tan cerca estan los estimados de los valores reales, comparando el valor real, el estimado de Maxima Verosimilitud (columnas 2 y 3) y el estimado Bayesiano (columnas 4 y 5). 
...
# Información de la sessión de R y los paquetes usados
